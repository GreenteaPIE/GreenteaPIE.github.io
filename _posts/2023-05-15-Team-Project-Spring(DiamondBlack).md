...
SpringBoot 로 넘어가기전 Spring 과 FrameWork를 배웠지만 본격적인 다음 팀프로젝트는 Boot로 진행 한다고 하여,<br>기존에 만든 프로젝트를 Spring 형식에 맞춰서 구현하고 싶었지만 못 만들었던 기능들을 추가하고, 아쉬웠던 <br>기능들의 디테일들을 살려 만들어 보기로 했다.
...
기존에 있던 DB에서 구현할 기능들에 필요한 컬럼들을 정리 후 추가하고 스프링에 설정할 dependency들과 기본적인<br>설정을 마친 후 설계한 API를 토대로 기존의 form들을 가져와 수정하고 추가한다. 
